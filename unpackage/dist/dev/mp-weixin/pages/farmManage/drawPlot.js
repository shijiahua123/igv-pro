...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const closeMapConfig = () => {
      common_vendor.index.__f__("log", "at pages/farmManage/drawPlot.vue:281", "关闭图层");
    ...
    const handleSave = () => {
      ...
      common_vendor.index.__f__("log", "at pages/farmManage/drawPlot.vue:701", "保存标记数据：", markData);
      common_vendor.index.showToast({
        title: "保存成功",
        icon: "success"
      });
    ...
    const checkPermission = () => {
      return new Promise((resolve, reject) => {
        common_vendor.index.getSetting({
          ...
          fail: (err) => {
            common_vendor.index.__f__("error", "at pages/farmManage/drawPlot.vue:724", "获取设置失败：", err);
            reject(err);
          }
        });
      });
    ...
    const requestPermission = () => {
      return new Promise((resolve, reject) => {
        common_vendor.index.authorize({
          ...
          fail: (err) => {
            common_vendor.index.__f__("error", "at pages/farmManage/drawPlot.vue:740", "权限请求失败：", err);
    ...
    const getLocationInfo = () => {
      return new Promise((resolve, reject) => {
        common_vendor.index.getLocation({
          ...
          fail: (err) => {
            common_vendor.index.__f__("error", "at pages/farmManage/drawPlot.vue:778", "获取位置失败：", err);
            reject(err);
          }
        });
      });
    ...
    const getCurrentLocation = async () => {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/farmManage/drawPlot.vue:813", "位置服务异常：", err);
        error.value = "获取位置失败，请检查定位服务是否开启";
        isLoading.value = false;
      }
    };
    const handleRegionChange = (e) => {
      common_vendor.index.__f__("log", "at pages/farmManage/drawPlot.vue:821", "地图区域改变", e);
    };
    const handleMapUpdated = (e) => {
      common_vendor.index.__f__("log", "at pages/farmManage/drawPlot.vue:826", "地图更新完成", e);
      isLoading.value = false;
    };
    const handleMapError = (e) => {
      common_vendor.index.__f__("error", "at pages/farmManage/drawPlot.vue:832", "地图加载错误：", e);
    ...
    const handleLayerClick = () => {
      common_vendor.index.__f__("log", "at pages/farmManage/drawPlot.vue:844", "点击图层按钮");
...
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/farmManage/drawPlot.js.map
